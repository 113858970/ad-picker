...
exports.default = {
  ...
  methods: {
    ...
    moveTo: function moveTo(el, y) {
      ...
      var time =
        arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0.3;
    ...
    initDate: function initDate() {
      ...
      if (now.toString().toLowerCase().indexOf("invalid") !== -1) {
        ...
        if (this.date === "date") {
          ...
          if (arr && arr.length === 2) {
            name =
              arr[0].length === 1 ? "0" + arr[0] + "\u65F6" : arr[0] + "\u65F6";
            name +=
              arr[1].length === 1 ? "0" + arr[1] + "\u5206" : arr[1] + "\u5206";
          }
        } else if (this.date === "datetime") {
          name = name.replace(
            /(\d+)\-(\d+)\-(\d+) (\d+):(\d+)/g,
    ...
    setMinutes: function setMinutes(nowMinutes) {
      ...
      while (minutes < 60) {
        minutesColum.push({
          name:
            ((minutes + "").length === 1 ? "0" + minutes : minutes) + "\u5206",
          id: minutes,
        });
        minutes += 1;
      }
